...
def analyze_audio(path, unique_id):  
    ...
    with st.expander("🛠 Edit Audio Parameters", expanded=True):  
        ...
        zoom_start, zoom_end = st.slider(  
            "Zoom waveform (seconds)", 0.0, float(duration_slider),  
            (0.0, float(duration_slider)), step=0.1, key=f"zoom_{unique_id}"  
        )  
