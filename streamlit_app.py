import streamlit as st import librosa import librosa.display import numpy as np import matplotlib.pyplot as plt import soundfile as sf import noisereduce as nr from fpdf import FPDF import io import base64

--------------------- Streamlit UI Setup ---------------------

st.set_page_config(page_title="Heartest - Heart Sound Analyzer", layout="wide") st.title("💓 Heartest - Heart Sound Analyzer")

st.sidebar.header("🩺 New Case Entry") name = st.sidebar.text_input("Patient Name") age = st.sidebar.text_input("Age") gender = st.sidebar.selectbox("Gender", ["Male", "Female", "Other"]) symptoms = st.sidebar.text_area("Symptoms")

Upload Section

st.subheader("📤 Upload Heart Sound (.wav file)") uploaded_file = st.file_uploader("Choose a .wav file", type=["wav"])

if uploaded_file is not None: st.audio(uploaded_file, format='audio/wav')
...
else: st.warning("Upload a heart sound .wav file to begin analysis.")

Footer

st.markdown("---") st.markdown("Developed by JV-GIRI | Powered by Streamlit")
